...
import numpy as np
import time, threading, sys, glob, math
import csv
...
def main():
    ...
    try:
        ...
        while True:
            ...
            data_buffer[-1] = new_point

            global COUNT
            COUNT += 250*0.02
            
            if COUNT >= WINDOW_SIZE:
                # Calculate SNR along axis 0 (columns)
                snr = signaltonoise(data_buffer, axis=0, ddof=0)
                print(f"SNR along last 4 seconds: {snr}")
                magnitude = 0.5  # Example magnitude
                decibels_magnitude = 20 * math.log10(magnitude)
                print(f"Decibels (magnitude): {decibels_magnitude:.2f} dB")
            #======
            #======
