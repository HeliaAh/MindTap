...
# ===== CONFIG =====
BOARD_ID     = BoardIds.SYNTHETIC_BOARD   # use SYNTHETIC_BOARD for a quick test
...
def test_impedance_brainflow(board, ch, fs):
    """Test impedance for a single channel on a Cyton board."""
    try:
        cmd = f"z{ch}Z"
        ...
        if data.shape[1] < fs // 2:
            return impedance_values[ch]  # keep last value if not enough data

        sig = data[eeg_channels[ch - 1], :]
        b, a = signal.butter(2, [30/(fs/2), 33/(fs/2)], btype='band')
        filtered = signal.lfilter(b, a, sig)
        v_rms = np.sqrt(np.mean(filtered ** 2))

        z = (1.414 * v_rms * 1e6) / 6e-9 - 2200  # in Ohms
        return max(z / 1000.0, 0.0)  # return in kΩ
    except Exception as e:
        print(f"[impedance] ch{ch} error:", e)
        return impedance_values[ch]


def impedance_worker(board, fs):
    """Background thread: continuously update impedance every few seconds."""
    global impedance_values
    eeg_channels = board.get_eeg_channels(BoardIds.CYTON_BOARD.value)
    b, a = signal.butter(2, [30/(fs/2), 33/(fs/2)], btype='band')
    ...
    while True:
        try:
            if BOARD_ID == BoardIds.SYNTHETIC_BOARD.value:
                # Fake impedance for testing
                for ch in range(1, USE_N_CHANS+1):
                    target = np.random.uniform(5, 25)
                    impedance_values[ch] = (
                        smoothing * impedance_values[ch] + (1 - smoothing) * target
                    )
                time.sleep(1.0)
                continue

            # Real Cyton impedance update
            for ch in range(1, USE_N_CHANS+1):
                z_kohm = test_impedance_brainflow(board, ch, fs)
                # Smooth transitions
                impedance_values[ch] = (
                    smoothing * impedance_values[ch] + (1 - smoothing) * z_kohm
                )
                time.sleep(0.1)

            time.sleep(4.0)  # re-check every ~4s

        except Exception as e:
            print("[impedance_worker] Error:", e)
            time.sleep(1.0)
...
def main():
    ...
    try:
        ...
        while True:
            ...
            threading.Thread(target=impedance_worker, args=(board, fs), daemon=True).start()

            imp_text = " | ".join([f"Ch{ch}:{impedance_values[ch]:.1f}kΩ" for ch in range(1,USE_N_CHANS+1)])
            print(f"[IMP] {imp_text}")
